...
export default function ExportLTVAnalysisButton() {
            ...
            try {
                ...
                if (statusData.state === 'SUCCESS') {
                    ...
                    if (result && result.file_path) {
                        ...
                        toast.success(
                            `LTV анализ создан! Клиентов: ${result.customers_count}, Средняя LTV: ${result.average_ltv?.toLocaleString('ru-RU', { maximumFractionDigits: 2 })} ₽`,
                            { duration: 5000 }
                        );
    ...
    const handleExport = async () => {
        setIsLoading(true);
        setProgressMessage('Запуск LTV анализа...');
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/api/sales/reports/ltv-analysis/`, {
                method: 'POST',
                headers: {
            ...
            toast.info('LTV анализ запущен. Ожидайте...', { duration: 3000 });
            setProgressMessage('Анализ пожизненной ценности клиентов...');
        ...
        <div className="flex flex-col gap-2">
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>Параметры LTV анализа клиентов</DialogTitle>
                        <DialogDescription>
                            Анализ пожизненной ценности клиентов (Customer Lifetime Value)
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div className="grid gap-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Начальная дата для фильтрации транзакций
                            </p>
                        </div>
                        ...
                        <div className="grid gap-2">
                            ...
                            />
                            <p className="text-xs text-muted-foreground">
                                Конечная дата для фильтрации транзакций
                            </p>
                        </div>

                        <div className="text-sm text-muted-foreground space-y-1">
                            <p><strong>Historical LTV</strong> - реальная выручка от клиента</p>
                            <p><strong>AOV</strong> - средний чек клиента</p>
                            <p><strong>Purchase Frequency</strong> - частота покупок</p>
                            <p><strong>Predicted LTV</strong> - прогнозная ценность (12м/24м)</p>
                            <p className="pt-2">• 6 листов: LTV данные, Статистика, Сегменты, Активность, Параметры, Пояснения</p>
                            <p>• Сегментация: High Value, Medium Value, Low Value</p>
                            <p>• Статусы: Active, At Risk, Inactive, Churned</p>
                        </div>
                    </div>
