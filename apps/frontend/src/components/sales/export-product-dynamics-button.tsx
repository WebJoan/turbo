...
export default function ExportProductDynamicsButton() {
            ...
            try {
                ...
                if (statusData.state === 'SUCCESS') {
                    ...
                    if (result && result.file_path) {
                        ...
                        toast.success(
                            `Отчет создан! Товаров: ${result.products_count}, Записей: ${result.records_count}`,
                            { duration: 5000 }
                        );
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await fetch(`${apiUrl}/api/sales/reports/product-dynamics/`, {
                method: 'POST',
                headers: {
        ...
        <div className="flex flex-col gap-2">
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        <DialogTitle>Параметры отчета по динамике продаж товаров</DialogTitle>
                        <DialogDescription>
                            Настройте параметры для анализа выручки, количества заказов и среднего чека по товарам
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        ...
                        <div className="text-sm text-muted-foreground">
                            ...
                            <p>• Отчет включает 3 листа: детальная динамика, сводная по товарам и параметры</p>
                            <p>• Включает: выручку, заказы, проданные единицы, среднюю цену, средний чек</p>
                        </div>
                    </div>
