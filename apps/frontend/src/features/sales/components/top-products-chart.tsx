...
import { fetchTopProducts } from '@/lib/sales';
...
export function TopProductsChart({
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                setLoading(true);
                setError(null);
                const result = await fetchTopProducts(filters, limit);
                if (isMounted) {
                    setData(result);
                }
            } catch (err) {
    ...
    if (loading) {
        return (
            <Card>
                ...
                <CardContent>
                    <Skeleton className="h-[400px] md:h-[640px] w-full" />
                </CardContent>
            </Card>
        );
    }
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>Топ {limit} товаров по выручке</CardTitle>
                <CardDescription>
                    Ключевые товары с наибольшей выручкой за выбранный период.
                </CardDescription>
            </CardHeader>
            <CardContent>
                {chartData.length === 0 ? (
